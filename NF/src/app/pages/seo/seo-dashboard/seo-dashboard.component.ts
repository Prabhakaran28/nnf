      ...
      this.service.postData(environment.getBacklinkReportData, resourceFormData).subscribe(
        (res: Res) => {
          if (res.return_code != 0) {
            ...
            var title = 'Backlink by Resource (' + clickedData[0] + ")";
    ...
    this.service.postData(environment.getBacklinkReportData, siteFormData).subscribe(
      (res: Res) => {
        if (res.return_code != 0) {
          ...
          this.pieChartData.dataTable.push([label, data, { role: 'annotation' }, { role: 'style' }]);
          var i = 0;
          res.data.forEach(element => {
            labelarray.push(element[label]);
            dataarray.push(element[data]);
            this.pieChartData.dataTable.push([element[label], element[data],
