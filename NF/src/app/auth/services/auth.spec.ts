...
describe('auth-service', () => {
  ...
  it('requestPassword failed', (done) => {
      const spy = spyOn(dummyAuthProvider, 'forgotPassword')
        .and
        .returnValue(observableOf(failResult)
          .pipe(
            delay(1000),
      ...
      authService.forgotPassword('dummy').subscribe((authRes: NbAuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('requestPassword succeed', (done) => {
      const providerLogoutSpy = spyOn(dummyAuthProvider, 'forgotPassword')
        .and
