...
import { NbAuthToken, NbAuthJWTToken } from './token/token';
...
export class NbAuthService {
  ...
  isAuthenticated(): Observable<boolean> {
    //console.log("Call in isAuthen in auth svc");
    return this.getToken()
      .pipe(map((token1: NbAuthJWTToken) => token1.isJwtValid()));
  }
   ...
   * @returns {Observable<NbAuthResult>}
   */
  changePassword(provider: string, data?: any): Observable<NbAuthResult> {
    return this.getProvider(provider).changePassword(data).pipe(
      switchMap((result: NbAuthResult) => {
        if (result.isSuccess()) {
          this.tokenService.clear()
            .pipe(map(() => result));
   ...
   * @returns {Observable<NbAuthResult>}
   */
  forgotPassword(provider: string, data?: any): Observable<NbAuthResult> {
    return this.getProvider(provider).forgotPassword(data);
  }
   ...
   */
  protected getProvider(provider: string): NbEmailPassAuthProvider {
